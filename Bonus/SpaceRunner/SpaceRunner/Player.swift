...
class Player: SKSpriteNode {
    ...
    private let engine = GameParticles.sharedInstance.createParticles(particles: .engine)
    ...
    private func setup() {
        ...
        engine.alpha = 0.0
        engine.zPosition = self.zPosition - 1
    ...
    private func setupPhysics() {
        ...
        self.physicsBody?.contactTestBitMask = Contact.meteor | Contact.star | Contact.silverStar | Contact.shield | Contact.missile
    }
    ...
    private func fireEngine() {
        engine.alpha = 1.0
        
        engine.run(SKAction.wait(forDuration: 0.3), completion: {
            [weak self] in
            self?.engine.alpha = 0.0
        })
    }
    ...
    private func increaseScore(bonus: Int) {
        ...
        let bonusLabel = GameFonts.sharedInstance.createLabel(string: String(bonus), type: .bonus)
        bonusLabel.position = self.position
        self.parent?.addChild(bonusLabel)
        bonusLabel.run(GameFonts.sharedInstance.animate(label: bonusLabel))
    ...
    func pickupShield() {
        ...
        let bonusLabel = GameFonts.sharedInstance.createLabel(string: "+Shields", type: .message)
        bonusLabel.position = self.position
        self.parent?.addChild(bonusLabel)
        bonusLabel.run(GameFonts.sharedInstance.animate(label: bonusLabel))
