...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    private func setup() {
        ...
        gameNode.addChild(pickupController)
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if state != .running {
            ...
            if player.getLives() > 0 {
                ...
                pickupController.update(delta: delta)
            } else {
                stateGameOver()
            }
        }
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        if state != .running {
            ...
            if other.categoryBitMask == Contact.silverStar {
                if let silverStar = other.node as? SilverStar {
                    silverStar.contact()
                    player.pickupBonus()
                    statusBar.updateScore(score: player.getScore())
            ...
            if other.categoryBitMask == Contact.shield {
                if let shield = other.node as? Shield {
                    shield.contact()
                    player.pickupShield()
                }
            }
    ...
    private func animateExplosion() {
        explosion.animate(position: player.position)
    }
    
    private func shakeScreen() {
        gameNode.run(SKAction.shake(amount: CGPoint(x: 12, y: 12), duration: 1.0))
    }
